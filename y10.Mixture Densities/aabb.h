...
public:
	...
	bool hit(const ray& r, interval ray_t) const
	{
		...
		for (int axis = 0; axis < 3; axis++)
		{
			...
			// 参照：referrence/aabb.jpg の特殊ケース3において、掠入射角(grazing angle)の衝突判定は
			// falseではなくtrueと判定する必要があるため、パディング処理を適用
	...
	private:
		void padding_box()
		{
			// 面プリミティブにパディングを追加し、数学的エラー（ゼロ除算など）を防止
			double delta = 0.0001;
...
// クラスと同じ型の静的変数の定義はクラス外部で行われ、ここでメモリが割り当てられます
