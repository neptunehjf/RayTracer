    ...
    color ray_color(const ray& r, const hittable_list& scene, int bounce_count)
    {
        ...
        if (scene.hit(r, ray_t, rec))
        {
            ...
            if (rec.mat->scatter(r, rec, attenuation, scattered, pdf_value))
            {
				...
				// 
                // sample_pdfとscatter_pdfが完全に同じ場合、実質的には重点サンプリングを行わず 形式的に重点サンプリングの形にしただけになる
                // sample_pdfをscatter_pdfに設定することで、デバッグ時にサンプリングの影響を排除し散乱関数自体の問題を判断できる
